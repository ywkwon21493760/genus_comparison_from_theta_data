# -*- coding: utf-8 -*-
"""
Nonlinear and neural experiments for the genus-comparison benchmark.

This script runs the nonlinear and neural models reported in the manuscript:
  - DecisionTree
  - RandomForest
  - pair-feature MLP
  - two-channel Transformer

The tree-based models and MLP use the canonical pair-feature representation
  [u; v; |u - v|].
The Transformer uses the two-channel sequence representation
  ((u_n, v_n))_{n=0}^{P-1}.

The reported learned models use deterministic canonical pair ordering. They
should not be read as exact swap-invariant architectures.
...
# Model options
RUN_TRANSFORMER = True  # Set False to run only DecisionTree, RandomForest, and MLP.
...
# Optional theta parallelization parameters. The current implementation is single-process.
THETA_N_JOBS = 0
THETA_CHUNK = 500
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run nonlinear and neural experiments for the genus-comparison benchmark."
    )
    parser.add_argument(
        "--cache-dir",
        type=str,
    ...
    parser.add_argument(
        "--outdir",
        type=str,
        default="results/nonlinear_neural",
        help="Directory in which result CSV files and run_config_neural.json are written.",
    )
    parser.add_argument(
        "--seeds",
        type=int,
    ...
    parser.add_argument(
        "--no-transformer",
        action="store_true",
        help="Skip Transformer runs and run only DecisionTree, RandomForest, and MLP.",
    )
    parser.add_argument(
        "--no-disk-cache",
        action="store_true",
...
def write_run_config(outdir: str, device: torch.device) -> None:
    config = {
        "script": "run_nonlinear_neural.py",
        ...
        "models": ["DecisionTree", "RandomForest", "MLP"] + (["Transformer"] if RUN_TRANSFORMER else []),
        "decision_tree": {"max_depth": 25},
        ...
        "transformer": {
            ...
            "lr": TF_LR,
            "batch_size": TF_BATCH,
            "max_epochs_by_p": TF_MAX_EPOCHS_BY_P,
            "patience": TF_PATIENCE,
            "min_epochs": TF_MIN_EPOCHS,
        },
        "outputs": {
            "per_seed": "results_neural_per_seed.csv",
            "summary": "results_neural_summary_mean_std.csv",
        },
    }
    os.makedirs(outdir, exist_ok=True)
    with open(os.path.join(outdir, "run_config_neural.json"), "w", encoding="utf-8") as f:
...
def main():
    ...
    global CACHE_DIR, SEEDS, P_LIST, MAX_P, RUN_TRANSFORMER, USE_DISK_CACHE
    CACHE_DIR = args.cache_dir
    SEEDS = args.seeds
    P_LIST = args.p_list
    MAX_P = max(P_LIST)
    RUN_TRANSFORMER = not args.no_transformer
    ...
    write_run_config(outdir, device)

    out_per_seed = os.path.join(outdir, "results_neural_per_seed.csv")
    out_summary = os.path.join(outdir, "results_neural_summary_mean_std.csv")
